...
class L_edge(nn.Module):
    ...
    def __init__(self):
        ...
        kernel = torch.FloatTensor( [[-1,-1,-1],[-1,9,-1],[-1,-1,-1]]).cuda().unsqueeze(0).unsqueeze(0)
    ...
    def forward(self, org , enhance ):
        ...
        D_org = org_pool
        D_enhance = F.conv2d(enhance_pool , self.weight, padding=1)

        E = 1 / (torch.pow(D_org - D_enhance,2)+1e-7)
        #description : let enhanced image spatial information be preserved.
        return E
'''
