...
def hdrgeneration(image_path, dest_path, src_path):
    ...
    HDRNet.load_state_dict(torch.load(
        "snapshots_weight_trongan93_RoIDE-Net_8_inter/RoIDE-Net_HDR_net_Epoch_Epoch250.pth"))

    Fusion_net = fusenet.Fusion_module(channels=3, r=2).cuda().eval()
    Fusion_net.load_state_dict(torch.load(
        "snapshots_weight_trongan93_RoIDE-Net_8_inter/RoIDE-Net_Fusion_net_Epoch_Epoch250.pth"))
...
if __name__ == '__main__':
    with torch.no_grad():
        ...
        target_path = 'test_output_seaport/set_6_RoIDE_final/'
